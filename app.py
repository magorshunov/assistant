            ...
            if delta.code_interpreter.outputs:
                for output in delta.code_interpreter.outputs:
                    if output.type == "logs":
                        ...
                        # Handle file outputs (e.g., images)
                        with st.chat_message("Assistant"):
                            file_content = client.files.content(output.file_id)
                            content_type = file_content.response.headers["content-type"]
                            if "image" in content_type:
                                image_bytes = file_content.content
...
def on_tool_call_done(self, tool_call):
    ...
    if tool_call.type == "code_interpreter":
        ...
        for output in tool_call.code_interpreter.outputs:
            if output.type == "logs":
                output_logs = f"### code interpreter\noutput:\n```\n{output.logs}\n```"
                with st.chat_message("Assistant"):
                    st.markdown(output_logs, True)
                    st.session_state.chat_log.append({"name": "assistant", "msg": output_logs})
            elif output.type == "file":
                with st.chat_message("Assistant"):
                    file_content = client.files.content(output.file_id)
                    content_type = file_content.response.headers["content-type"]
                    if "image" in content_type:
                        ...
                        st.session_state.chat_log.append({"name": "assistant", "msg": "![Image]()"})
                    else:
                        # Handle other file types if needed
                        pass
